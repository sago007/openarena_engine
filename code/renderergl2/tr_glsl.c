...
")), 0.0);\r\n#endif\r\n\r\n\tintensity *= fade;\r\n#if defined(USE_DEPTH)\r"
"\n  #if defined(USE_PCF)\r\n\tfloat part;\r\n\t\r\n\tdist = sampleDistMap(u"
"_ShadowMap, st + vec2(-1.0/256.0, -1.0/256.0), u_LightRadius);\r\n\tpart = "
" max(sign(lightDist - dist), 0.0);\r\n\r\n\tdist = sampleDistMap(u_ShadowMa"
"p, st + vec2( 1.0/256.0, -1.0/256.0), u_LightRadius);\r\n\tpart += max(sign"
"(lightDist - dist), 0.0);\r\n\r\n\tdist = sampleDistMap(u_ShadowMap, st + v"
"ec2(-1.0/256.0,  1.0/256.0), u_LightRadius);\r\n\tpart += max(sign(lightDis"
"t - dist), 0.0);\r\n\r\n\tdist = sampleDistMap(u_ShadowMap, st + vec2( 1.0/"
"256.0,  1.0/256.0), u_LightRadius);\r\n\tpart += max(sign(lightDist - dist)"
", 0.0);\r\n\r\n    #if defined(USE_DISCARD)\r\n\tif (part <= 0.0)\r\n\t{\r"
"\n\t\tdiscard;\r\n\t}\r\n    #endif\r\n\r\n\tintensity *= part * 0.25;\r\n "
" #else\r\n\tdist = sampleDistMap(u_ShadowMap, st, u_LightRadius);\r\n\r\n  "
"  #if defined(USE_DISCARD)\r\n\tif (lightDist - dist <= 0.0)\r\n\t{\r\n\t\t"
"discard;\r\n\t}\r\n   #endif\r\n\t\t\t\r\n\t//intensity *= max(sign(254.0 /"
" 255.0 - dist / u_LightRadius), 0.0);\r\n\tintensity *= max(sign(lightDist "
"- dist), 0.0);\r\n  #endif\r\n#else\r\n\tfloat part;\r\n\t\r\n\tdist = text"
"ure2D(u_ShadowMap, st + vec2(-1.0/256.0, -1.0/256.0)).x;\r\n\tpart += dist;"
"\r\n\r\n\tdist = texture2D(u_ShadowMap, st + vec2( 1.0/256.0, -1.0/256.0))."
"x;\r\n\tpart += dist;\r\n\r\n\tdist = texture2D(u_ShadowMap, st + vec2(-1.0"
"/256.0,  1.0/256.0)).x;\r\n\tpart += dist;\r\n\r\n\tdist = texture2D(u_Shad"
"owMap, st + vec2( 1.0/256.0,  1.0/256.0)).x;\r\n\tpart += dist;\r\n\t\r\n\t"
"intensity *= 1 - part * 0.25;\r\n#endif\r\n\t\t\r\n\tgl_FragColor.rgb = vec"
...
void GLSL_InitGPUShaders(void)
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (!GLSL_InitGPUShader(&tr.shadowmapShader[i], "shadowfill", attribs, qtrue, extradefines, qtrue, fallbackShadowfillShader_vp, fallbackShadowfillShader_fp, GENERIC_UNIFORM_COUNT))
		{
			ri.Error(ERR_FATAL, "Could not load shadowfill shader!\n");
		}

		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_DEFORMGEN,                 "u_DeformGen",                 GLSL_INT);
		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_DEFORMPARAMS,              "u_DeformParams",              GLSL_FLOAT5);
		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_TIME,                      "u_Time",                      GLSL_FLOAT);
		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_MODELVIEWPROJECTIONMATRIX, "u_ModelViewProjectionMatrix", GLSL_MAT16);
		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_MODELMATRIX,               "u_ModelMatrix",               GLSL_MAT16);
		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_VERTEXLERP,                "u_VertexLerp",                GLSL_FLOAT);
		
		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_LIGHTORIGIN,               "u_LightOrigin",               GLSL_VEC4);
		GLSL_AddUniform(&tr.shadowmapShader[i], GENERIC_UNIFORM_LIGHTRADIUS,               "u_LightRadius",               GLSL_FLOAT);

		GLSL_EndUniforms(&tr.shadowmapShader[i]);
