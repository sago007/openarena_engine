...
surfaceType_t	farPlaneSurface = SF_FAR_PLANE;
...
static qboolean SurfIsOffscreen( const drawSurf_t *drawSurf, vec4_t clipDest[128] ) {
	...
	tesselate( TESS_COUNT, drawSurf->surface );

	assert( tess.numVertexes < 128 );

	RB_AllocateSurface( );
	tesselate( TESS_ALL, drawSurf->surface );
	...
	numTriangles = tess.numIndexes[0] / 3;

	for ( i = 0; i < tess.numIndexes[0]; i += 3 )
	{
		vec3_t normal, vertexNormal;
		float dot;
		float len;

		VectorSubtract( tess.vertexPtr[tess.indexPtr[i]].xyz,
				tr.viewParms.or.origin, normal );
		...
		VectorCopy( tess.vertexPtr[tess.indexPtr[i]].normal,
			    vertexNormal );
...
qboolean R_MirrorViewBySurface (drawSurf_t *drawSurf, int entityNum) {
	...
	// draw stencil mask if stencil bits are available
	if( glGlobals.portalLevels > 0 ) {
		oldParms.isFirst = qfalse;
		tr.currentEntityNum = entityNum;
		tr.shiftedEntityNum = entityNum << QSORT_ENTITYNUM_SHIFT;
		R_AddDrawSurf( drawSurf->surface, tr.preparePortalShader, 0, 0 );
	...
	if( glGlobals.portalLevels > 0 ) {
		tr.currentEntityNum = entityNum;
		tr.shiftedEntityNum = entityNum << QSORT_ENTITYNUM_SHIFT;
		R_AddDrawSurf( drawSurf->surface, tr.finalisePortalShader, 0, 0 );
		R_AddDrawSurfCmd( tr.refdef.drawSurfs + tr.refdef.numDrawSurfs - 1, 1 );
	}
...
void R_GenerateDrawSurfs( void ) {
	// World surfaces have to be added first, IBO construction
	// depends on an empty drawSurf list
	R_AddWorldSurfaces ();
	...
	if( tr.copyDepthShader ) {
		R_AddDrawSurf( &farPlaneSurface, tr.copyDepthShader, 0, 0 );
	}
}
