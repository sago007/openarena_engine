...
static	void R_LoadLightmaps( lump_t *l, lump_t *surfs ) {
	...
	tr.numLightmaps = (tr.numLightmaps + tr.lightmapWidth * tr.lightmapHeight - 1)
		/ (tr.lightmapWidth * tr.lightmapHeight);

	tr.lightmaps = ri.Hunk_Alloc( tr.numLightmaps * sizeof(image_t *),
				      h_low );
	image = ri.Hunk_AllocateTempMemory( tr.lightmapWidth * tr.lightmapHeight *
	                                    LIGHTMAP_SIZE * LIGHTMAP_SIZE * 4 );
	Com_Memset( image, 0, tr.lightmapWidth * tr.lightmapHeight *
	            LIGHTMAP_SIZE * LIGHTMAP_SIZE * 4);
	...
	tr.lightmaps[i] = R_CreateImage( va("*lightmap%d",i), image, 
					 ...
					 qfalse, qfalse, GL_CLAMP_TO_EDGE );
	tr.deluxeOffset = 1.0f / tr.lightmapWidth;
...
static	void R_LoadSurfaces( lump_t *surfs, lump_t *verts, lump_t *indexLump ) {
	...
	if ( qglGenBuffersARB ) {
		...
		RB_BeginSurface( tr.buildVBOShader );
		
		for ( i = 0, out = sortBuffer; i < count; i++, out++ ) {
			switch( out->type ) {
			case SF_FACE:
				tesselate( TESS_COUNT, out->data );
				break;
			case SF_GRID:
				tesselate( TESS_COUNT, out->data );
				break;
		...
		for ( i = 0, out = sortBuffer; i < count; i++, out++ ) {
			...
			case SF_FACE:
				...
				tesselate( TESS_VERTEX, out->data );
				break;
			case SF_GRID:
...
void R_LoadLightGrid( lump_t *l ) {
	...
	// upload light grid to 3D texture
	if( backEnd.fogBuffer && qglTexImage3DEXT ) {
		int	z, maxz, planeSize;
		byte	*buffer, *in, *plane1, *plane2;

		qglTexImage3DEXT( GL_PROXY_TEXTURE_3D, 0, GL_RGBA8,
				  (int)w->lightGridBounds[0],
				  (int)w->lightGridBounds[1],
				  2 * (int)w->lightGridBounds[2],
				  0, GL_RGBA, GL_UNSIGNED_BYTE, NULL );
		qglGetTexLevelParameteriv(GL_PROXY_TEXTURE_3D, 0,
					  GL_TEXTURE_WIDTH, &i);
		if( i == 0 )
			return;

		buffer = ri.Hunk_AllocateTempMemory( numGridPoints * 8 );

		planeSize = w->lightGridBounds[0] * w->lightGridBounds[1];
		maxz = 2 * w->lightGridBounds[2] - 1;
		for( z = 0; z < w->lightGridBounds[2]; z++ ) {
			in = w->lightGridData + 8 * z * planeSize;
			plane1 = buffer + 4 * z * planeSize;
			plane2 = buffer + 4 * (maxz - z) * planeSize;
			for( i = 0; i < planeSize; i++ ) {
				int	lat, lng;
				vec3_t	normal;
				float	scale;

				lat = in[i * 8 + 7] * (FUNCTABLE_SIZE/256);
				lng = in[i * 8 + 6] * (FUNCTABLE_SIZE/256);
				normal[0] = tr.sinTable[(lat+(FUNCTABLE_SIZE/4))&FUNCTABLE_MASK] * tr.sinTable[lng];
				normal[1] = tr.sinTable[lat] * tr.sinTable[lng];
				normal[2] = tr.sinTable[(lng+(FUNCTABLE_SIZE/4))&FUNCTABLE_MASK];

				scale = 1.0f / (fabs(normal[0]) + fabs(normal[1]) + fabs(normal[2]));
				normal[0] *= scale;
				normal[1] *= scale;
				normal[2] *= scale;

				if( normal[2] < 0.0f ) {
					normal[0] = sign(normal[0]) - normal[0];
					normal[1] = sign(normal[1]) - normal[1];
				}

				plane1[i * 4 + 0] = in[i * 8 + 0];
				plane1[i * 4 + 1] = in[i * 8 + 1];
				plane1[i * 4 + 2] = in[i * 8 + 2];
				plane1[i * 4 + 3] = (byte)(normal[0] * 127.5f + 127.5f);
				plane2[i * 4 + 0] = in[i * 8 + 3];
				plane2[i * 4 + 1] = in[i * 8 + 4];
				plane2[i * 4 + 2] = in[i * 8 + 5];
				plane2[i * 4 + 3] = (byte)(normal[1] * 127.5f + 127.5f);
			}
		}

		qglGenTextures( 1, &tr.lightGridTexture );
		GL_ActiveTexture( TMU_LIGHTGRID );
		qglBindTexture( GL_TEXTURE_3D, tr.lightGridTexture );
		qglTexImage3DEXT( GL_TEXTURE_3D, 0, GL_RGBA8,
				  w->lightGridBounds[0],
				  w->lightGridBounds[1],
				  2 * w->lightGridBounds[2],
				  0, GL_RGBA, GL_UNSIGNED_BYTE, buffer );
		qglTexParameterf( GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER,
				  GL_LINEAR );
		qglTexParameterf( GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER,
				  GL_LINEAR );
		qglTexParameterf( GL_TEXTURE_3D, GL_TEXTURE_WRAP_S,
				  GL_CLAMP_TO_EDGE );
		qglTexParameterf( GL_TEXTURE_3D, GL_TEXTURE_WRAP_T,
				  GL_CLAMP_TO_EDGE );
		qglTexParameterf( GL_TEXTURE_3D, GL_TEXTURE_WRAP_R,
				  GL_CLAMP_TO_EDGE );
...
/*
 */
static void BuildFogBuffer( void ) {
	int	i;
	byte	*buffer;
	float	*ptrLightGridScale, *ptrLightGridOffset, *ptrColor, *ptrPlane;

	if( backEnd.fogBuffer ) {
		GL_UBO( backEnd.fogBuffer );
		buffer = qglMapBufferARB( GL_UNIFORM_BUFFER, GL_WRITE_ONLY_ARB );
		ptrLightGridScale = (float *)(buffer + backEnd.uLightGridScale.offset);
		ptrLightGridOffset = (float *)(buffer + backEnd.uLightGridOffset.offset);
		ptrColor = (float *)(buffer + backEnd.uFogColors.offset);
		ptrPlane = (float *)(buffer + backEnd.uFogPlanes.offset);

		if( !tr.lightGridTexture ) {
			ptrLightGridScale[0] = 0.0f;
			ptrLightGridScale[1] = 0.0f;
			ptrLightGridScale[2] = 0.0f;
			ptrLightGridOffset[0] = 0.0f;
			ptrLightGridOffset[1] = 0.0f;
			ptrLightGridOffset[2] = 0.0f;
		} else {
			ptrLightGridScale[0] = s_worldData.lightGridInverseSize[0] / s_worldData.lightGridBounds[0];
			ptrLightGridScale[1] = s_worldData.lightGridInverseSize[1] / s_worldData.lightGridBounds[1];
			ptrLightGridScale[2] = 0.5f * s_worldData.lightGridInverseSize[2] / s_worldData.lightGridBounds[2];
			ptrLightGridOffset[0] = (s_worldData.lightGridOrigin[0] - 0.5 * s_worldData.lightGridSize[0])
				* ptrLightGridScale[0];
			ptrLightGridOffset[1] = (s_worldData.lightGridOrigin[1] - 0.5 * s_worldData.lightGridSize[1])
				* ptrLightGridScale[1];
			ptrLightGridOffset[2] = (s_worldData.lightGridOrigin[2] - 0.5 * s_worldData.lightGridSize[2])
				* ptrLightGridScale[2];
		}

		for(i = 1; i < s_worldData.numfogs; i++ ) {
			ptrColor[0] = s_worldData.fogs[i].parms.color[0];
			ptrColor[1] = s_worldData.fogs[i].parms.color[1];
			ptrColor[2] = s_worldData.fogs[i].parms.color[2];
			ptrColor[3] = 8.0f * s_worldData.fogs[i].tcScale;

			if( s_worldData.fogs[i].hasSurface ) {
				ptrPlane[0] = s_worldData.fogs[i].surface[0];
				ptrPlane[1] = s_worldData.fogs[i].surface[1];
				ptrPlane[2] = s_worldData.fogs[i].surface[2];
				ptrPlane[3] = s_worldData.fogs[i].surface[3];
			} else {
				ptrPlane[0] = 0.0f;
				ptrPlane[1] = 0.0f;
				ptrPlane[2] = 0.0f;
			...
			ptrColor = (float *)((byte *)ptrColor + backEnd.uFogColors.stride);
			ptrPlane = (float *)((byte *)ptrPlane + backEnd.uFogPlanes.stride);
		}
		
		qglUnmapBufferARB( GL_UNIFORM_BUFFER );
		GL_UBO( 0 );

		qglBindBufferBase( GL_UNIFORM_BUFFER, 2, backEnd.fogBuffer );
	}
}
