...
typedef struct image_s {
	...
	GLuint		buffer;			// VBO for buffer textures
	...
	qboolean        hasAlpha;
	GLint		wrapClampMode;		// GL_CLAMP_TO_EDGE or GL_REPEAT
...
typedef enum {
	...
	SS_COPYDEPTH,		// copy depth to texture
...
// TMUs 0-7 may be used by shader stages, 8 and higher can be allocated globally
#define TMU_LIGHTGRID 8
...
typedef struct iboInfo_s {
	struct iboInfo_s *next;
	vboInfo_t	*vbo;
	GLuint  ibo;
	int	numIndexes;
	int     minIndex;
...
// information about a uniform in an uniform buffer object
typedef struct BufUniform_s {
	GLuint	offset;
	GLuint	stride;
} BufUniform_t;

// flags describing how the GL attributes (vertex, normal, tc1, tc2 and color)
// are bound in a shader
typedef enum {
	GLA_COLOR_dynamic	= 0x0001,
	GLA_COLOR_vtxcolor	= 0x0002,
	GLA_COLOR_uniform	= 0x0004,
	GLA_COLOR_mask		= 0x0007,

	GLA_TC1_dynamic		= 0x0008,
	GLA_TC1_texcoord	= 0x0010,
	GLA_TC1_lmcoord		= 0x0020,
	GLA_TC1_uniform		= 0x0040,
	GLA_TC1_mask		= 0x0078,

	GLA_TC2_dynamic		= 0x0080,
	GLA_TC2_texcoord	= 0x0100,
	GLA_TC2_lmcoord		= 0x0200,
	GLA_TC2_uniform		= 0x0400,
	GLA_TC2_mask		= 0x0780,

	GLA_NORMAL_dynamic	= 0x1000,
	GLA_VERTEX_dynamic	= 0x2000,
	GLA_FULL_dynamic        = 0x4000
} GLAttrFlag;
// shift GLA_TC_shift bits to get the next TC bundle
#define GLA_TC_shift 4
...
// data loaded into an IBO/VBO
typedef struct srfIBO_s {
	surfaceType_t	surfaceType;
	shader_t	*shader;
	iboInfo_t	ibo;
} srfIBO_t;

typedef enum tessMode_e {
	TESS_COUNT  = 0,
	TESS_VERTEX = 1,
	TESS_INDEX  = 2,
	TESS_ALL    = 3
...
#define tesselate( mode, surf ) rb_surfaceTable[ *(surf) ] ( (mode), (surf) )
extern	void (*rb_surfaceTable[SF_NUM_SURFACE_TYPES])(tessMode_t,
						      surfaceType_t *);
...
typedef struct {
	...
	int		numSurfaces;
	int		numIBOSurfaces;
	srfIBO_t	*iboSurfaces;
} bmodel_t;

typedef struct {
	vec3_t		mins, maxs;		// for bounding box culling
	int		numIBOSurfaces[7];
	srfIBO_t	*iboSurfaces[7];
} mcluster_t;
...
typedef struct model_s {
	...
	void		*modelData;		// only if type == (MOD_MD4 | MOD_MDR | MOD_IQM )
						// also MOD_MESH, if using precalculated VBOs
...
7-16  : entity index
5-6   : dlights value: 0 = no dlights, 1 = dlights and surface, 2 = dlight only
0-4   : fog index
*/
// sizes of the sort fields
#define QSORT_SHADERNUM_BITS	15
#define QSORT_ENTITYNUM_BITS	10
#define QSORT_DLIGHT_BITS	2
...
#define QSORT_DLIGHT_SHIFT	(QSORT_FOGNUM_SHIFT + QSORT_FOGNUM_BITS)
#define QSORT_ENTITYNUM_SHIFT	(QSORT_DLIGHT_SHIFT + QSORT_DLIGHT_BITS)
#define QSORT_SHADERNUM_SHIFT	(QSORT_ENTITYNUM_SHIFT + QSORT_ENTITYNUM_BITS)

#define QSORT_FOGNUM_MASK	((1 << QSORT_DLIGHT_SHIFT) - (1 << QSORT_FOGNUM_SHIFT))
#define QSORT_DLIGHT_MASK	((1 << QSORT_ENTITYNUM_SHIFT) - (1 << QSORT_DLIGHT_SHIFT))
...
// binding state of a single attribute
typedef struct {
	enum {
		RA_UNSPEC,
		RA_POINTER,
...
typedef struct {
	...
	qboolean	texEnabled[NUM_TEXTURE_BUNDLES];
	int		texEnv[NUM_TEXTURE_BUNDLES];
	int		faceCulling;
	unsigned long	glStateBits;
	GLint		activeTexture, clientActiveTexture;
  GLuint		currentVBO, currentIBO, currentUBO;
	GLSLprogram_t	*currentProgram;

	glAttribState_t	glAttribute[ AL_NUMATTRIBUTES ];
} glstate_t;
...
typedef struct {
	...
	glAttribState_t	attrib[AL_NUMATTRIBUTES];
} glRenderState_t;
#define InitState( st ) Com_Memset( (st), 0, sizeof(glRenderState_t) )
#define SetAttrPointerInst( st, ind, vbo, s, t, str, p, d ) \
	...
	(st)->attrib[ind].ptr = (p);	             \
	(st)->attrib[ind].divisor = (d)
#define SetAttrPointer( st, ind, vbo, s, t, str, p ) \
	SetAttrPointerInst( st, ind, vbo, s, t, str, p, 0 )
...
typedef struct glStreamBuffer_s {
	GLuint		id;
	qboolean	mapped;
	GLsizei		size;
	GLsizei		nextFree;
...
typedef struct {
	...
	GLuint		dlightBuffer;
	GLsizei		dlightBufferSize;
	BufUniform_t	uDLSpheres;
	BufUniform_t	uDLColors;
	BufUniform_t	uDLLinks;
	BufUniform_t	uDLNum;
	BufUniform_t	uDLDebug;

	GLuint		fogBuffer;
	GLsizei		fogBufferSize;
	BufUniform_t	uLightGridScale;
	BufUniform_t	uLightGridOffset;
	BufUniform_t	uFogPlanes;
	BufUniform_t	uFogColors;	// density in .w
} backEndState_t;
...
typedef struct {
	...
	shader_t				*buildVBOShader;
	shader_t				*buildIBOShader;
	...
	shader_t				*copyDepthShader;

	int						numLightmaps;
	qboolean				hasDeluxemaps;
	float					deluxeOffset;
	image_t					**lightmaps;
	GLuint				lightGridTexture;
...
extern cvar_t   *r_ext_map_buffer_range;
...
extern cvar_t   *r_ext_texture_buffer_object;
extern cvar_t   *r_ext_uniform_buffer_object;
...
extern cvar_t	*r_ext_blend_func_extended;
...
void R_SortSurfaces( drawSurf_t *source, int size );
...
void	GL_LockArrays( glRenderState_t * );
...
#define GLS_DSTBLEND_SRC1_COLOR				0x00000090
...
typedef struct shaderCommands_s 
{
	...
	// vertex and index data is tesselated into these buffers
	void		*vertexBuffer;
	glStreamBuffer_t	VtxBuf;
	glStreamBuffer_t	IdxBuf;
	...
	// data for vertex shader animation
	image_t		*dataTexture;
	image_t		*imgOverride;	// use this image for shader img
	int		framesPerRow;
	float		scaleX, scaleY;
	...
	// instancing
	int		numInstances;
	instanceVars_t	instances[ MAX_ENTITIES ];
...
void R_BuildCloudData( shaderCommands_t *input, shader_t *shader );
...
void RB_SurfaceAnim( tessMode_t mode, surfaceType_t *surfType );
#ifdef RAVENMD4
void R_MDRAddAnimSurfaces( trRefEntity_t *ent );
void RB_MDRSurfaceAnim( tessMode_t mode, surfaceType_t *surface );
#endif
qboolean R_LoadIQM (model_t *mod, void *buffer, int filesize, const char *name );
void R_AddIQMSurfaces( trRefEntity_t *ent );
void RB_IQMSurfaceAnim( tessMode_t mode, surfaceType_t *surface );
...
void	RB_CalcFogTexCoords( vec2_t *dstTexCoords, int skip, int numVertexes );
