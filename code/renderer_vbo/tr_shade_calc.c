...
void RB_CalcDeformVertexes( deformStage_t *ds )
{
	...
	if ( ds->deformationWave.frequency == 0 )
	{
		...
		{
			normal = &tess.vertexPtr[i].normal;
...
void RB_CalcDeformNormals( deformStage_t *ds ) {
	...
	for ( i = 0; i < tess.numVertexes; i++ ) {
		...
		scale = 0.98f;
		scale = R_NoiseGet4f( tess.vertexPtr[i].xyz[0] * scale,
				      tess.vertexPtr[i].xyz[1] * scale,
				      tess.vertexPtr[i].xyz[2] * scale,
			tess.shaderTime * ds->deformationWave.frequency );
		(*normal)[ 0 ] += ds->deformationWave.amplitude * scale;

		scale = 0.98f;
		scale = R_NoiseGet4f( 100 + tess.vertexPtr[i].xyz[0] * scale,
				      tess.vertexPtr[i].xyz[1] * scale,
				      tess.vertexPtr[i].xyz[2] * scale,
				      tess.shaderTime * ds->deformationWave.frequency );
		...
		scale = 0.98f;
		scale = R_NoiseGet4f( 200 + tess.vertexPtr[i].xyz[0] * scale,
				      tess.vertexPtr[i].xyz[1] * scale,
				      tess.vertexPtr[i].xyz[2] * scale,
				      tess.shaderTime * ds->deformationWave.frequency );
...
void RB_CalcBulgeVertexes( deformStage_t *ds ) {
	...
	for ( i = 0; i < tess.numVertexes; i++ ) {
		...
		normal = &tess.vertexPtr[i].normal;
		tess.vertexPtr[i].xyz[0] += (*normal)[0] * scale;
		tess.vertexPtr[i].xyz[1] += (*normal)[1] * scale;
		tess.vertexPtr[i].xyz[2] += (*normal)[2] * scale;
	}
}
...
void DeformText( const char *text ) {
	...
	height[1] = 0;
	height[2] = -1;
	normal = &tess.vertexPtr[0].normal;
	...
	tess.numVertexes = 4 * len;
	tess.numIndexes[0] = 6 * len;
	RB_AllocateSurface( );
...
static void AutospriteDeform( void ) {
	...
	if ( tess.numIndexes[1] != ( tess.numVertexes >> 2 ) * 6 ) {
...
void RB_CalcModulateColorsByFog( color4ub_t *colors, int numVertexes ) {
	...
	RB_CalcFogTexCoords( texCoords, 0, numVertexes );
...
========================
*/
void RB_CalcFogTexCoords( vec2_t *st, int skip, int numVertexes ) {
	int			i;
	float		s, t;
	float		eyeT;
	qboolean	eyeOutside;
	...
	for (i = skip, st += skip ; i < numVertexes ; i++, st++ ) {
...
void RB_CalcEnvironmentTexCoords( vec2_t *st, int numVertexes )
{
	...
	{
		VectorSubtract (backEnd.or.viewOrigin, tess.vertexPtr[i].xyz,
				viewer);
