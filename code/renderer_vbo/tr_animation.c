...
void R_AddAnimSurfaces( trRefEntity_t *ent ) {
	...
	for ( i = 0 ; i < lod->numSurfaces ; i++ ) {
		...
		R_AddDrawSurf( (void *)surface, shader, 0 /*fogNum*/, 0 );
...
==============
*/
void RB_SurfaceAnim( tessMode_t mode, surfaceType_t *surf ) {
	...
	if ( mode & TESS_VERTEX ) {
		...
		for ( j = 0; j < numVerts; j++ ) {
			...
			VectorCopy( tempVert, vertexPtr->xyz );
			vertexPtr->fogNum = (float)tess.fogNum;
			vertexPtr->tc1[0] = v->texCoords[0];
			vertexPtr->tc1[1] = v->texCoords[1];
			vertexPtr->tc2[0] = v->texCoords[0];
			vertexPtr->tc2[1] = v->texCoords[1];
			VectorCopy( tempNormal, vertexPtr->normal );
	...
	if ( mode & TESS_INDEX ) {
		GLuint *indexPtr = tess.indexPtr + tess.numIndexes[tess.indexRange];
	...
	tess.numIndexes[tess.indexRange] += surface->numTriangles * 3;
...
==============
*/
void RB_MDRSurfaceAnim( tessMode_t mode, surfaceType_t *surf )
{
	md4Surface_t *surface = (md4Surface_t *) surf;
	int				i, j, k;
	float			frontlerp, backlerp;
