...
static qboolean R_LoadMD3 (model_t *mod, int lod, void *buffer, const char *mod_name ) {
	...
	// VBO interpolation needs shaders, buffer objects, float textures
	// and multitexture
	if( qglCreateShader &&
	    qglGenBuffersARB &&
	    glGlobals.floatTextures &&
	    qglActiveTextureARB &&
	    glGlobals.maxVertexTextureImageUnits > 0 &&
	    mod->md3[lod]->numSurfaces > 0 ) {
		vboInfo_t	*VBO;
		GLuint		IBO;
		int frameW = 1;
		int frameH = 1;
		int texW = 1;
		int texH = 1;
		float   vrtScaleX, vrtScaleY, frmScaleX, frmScaleY;
		float	coordOffs;
		vec4_t *texData;
		srfMD3Texture_t	*(*data)[ MD3_MAX_LODS ] = mod->modelData;
		srfMD3Texture_t	*md3srf;
		...
		for ( i = 0 ; i < mod->md3[lod]->numSurfaces ; i++ ) {
			tesselate( TESS_COUNT, (surfaceType_t *)surf );
			surf = (md3Surface_t *)( (byte *)surf + surf->ofsEnd );
		}

		if( !data ) {
			data = ri.Hunk_Alloc( sizeof( *data )
					      + mod->md3[lod]->numSurfaces * sizeof( srfMD3Texture_t ), h_low );
			md3srf = (srfMD3Texture_t *)(data + 1);
			(*data)[lod] = md3srf;
			mod->modelData = data;
		} else {
			md3srf = ri.Hunk_Alloc( mod->md3[lod]->numSurfaces * sizeof(srfMD3Texture_t), h_low );
			(*data)[lod] = md3srf;
		}
		...
		// the first frame.
		if( qglTexBufferEXT ) {
			frameW = tess.numVertexes;
			frameH = 1;
			texW = numFrames;
			texH = 0;
			vrtScaleX = 1.0f;
			vrtScaleY = 0.0f;
			frmScaleX = (float)tess.numVertexes;
			frmScaleY = 0.0f;
			coordOffs = 0.0f;
		} else {
			while( frameW * frameH < tess.numVertexes ) {
				frameH *= 2;
				if( frameW * frameH >= tess.numVertexes )
					break;
				frameW *= 2;
			}
			...
			frmScaleX = 1.0f / texW;
			frmScaleY = 1.0 / texH;
			vrtScaleX = frmScaleX / frameW;
			vrtScaleY = frmScaleY / frameH;
			coordOffs = 0.5f;
		}
		...
		VBO = ri.Hunk_Alloc( sizeof( vboInfo_t ), h_dontcare );
		...
		surf = (md3Surface_t *) ( (byte *)mod->md3[lod] + mod->md3[lod]->ofsSurfaces );
		for ( i = 0 ; i < mod->md3[lod]->numSurfaces ; i++) {
			int	firstIndex = tess.numIndexes[0];

			md3srf[i].surfaceType = SF_MD3_TEXTURE;
			md3srf[i].framesPerRow = texW;
			md3srf[i].scaleX = frmScaleX;
			md3srf[i].scaleY = 1024.0f * frmScaleY;
			
			md3srf[i].IBO = ri.Hunk_Alloc( sizeof( iboInfo_t ), h_dontcare );
			md3srf[i].IBO->ibo = IBO;
			md3srf[i].IBO->offset = tess.numIndexes[0] + (glIndex_t *)NULL;
			
			md3srf[i].IBO->vbo = VBO;
			
			tesselate( TESS_INDEX, (surfaceType_t *)surf );

			md3srf[i].IBO->numIndexes = tess.numIndexes[0] - firstIndex;
			md3srf[i].IBO->minIndex = tess.minIndex[0];
			md3srf[i].IBO->maxIndex = tess.maxIndex[0];

			tess.maxIndex[0] = 0;
			tess.minIndex[0] = 0x7fffffff;
			
			surf = (md3Surface_t *)( (byte *)surf + surf->ofsEnd );
		}

		// allocate empty texture for vertex positions
		image = R_CreateImage( surf->name, NULL,
		...
		if( qglTexBufferEXT ) {
			...
			image->internalFormat = GL_RGBA32F_ARB;
			GL_VBO( image->buffer );
			qglBufferDataARB( GL_ARRAY_BUFFER_ARB,
					  frameW * texW * sizeof(vec4_t),
					  NULL, GL_STATIC_DRAW_ARB );
			texData = qglMapBufferARB( GL_ARRAY_BUFFER_ARB,
						   GL_WRITE_ONLY_ARB );
		} else {
			...
			texData = ri.Hunk_AllocateTempMemory(frameW * frameH * sizeof(vec4_t));
		}

		GL_VBO( md3srf[0].VBO->vbo );
		GL_IBO( md3srf[0].IBO->ibo );
		...
		for( k = 0; k < numFrames; k++ ) {
			...
			tess.numVertexes   = 0;
			for ( i = 0 ; i < mod->md3[lod]->numSurfaces ; i++) {
				md3srf[i].image = image;
				tesselate( TESS_VERTEX, (surfaceType_t *)surf );
				surf = (md3Surface_t *)( (byte *)surf + surf->ofsEnd );
			}
			...
			if( k == 0 ) {
				...
				ptr = qglMapBufferARB( GL_ARRAY_BUFFER_ARB,
						       GL_WRITE_ONLY_ARB );
				// build VBO, set 2nd texcoord (normally lightmap)
				// to position of vertex in the data texture
				for( j = 0; j < tess.numVertexes; j++ ) {
					ptr[j][0] = tess.vertexPtr[j].tc1[0];
					ptr[j][1] = tess.vertexPtr[j].tc1[1];
					ptr[j][2] = (coordOffs + (j % frameW)) * vrtScaleX;
					ptr[j][3] = (coordOffs + (j / frameW)) * vrtScaleY ;
				}
				qglUnmapBufferARB( GL_ARRAY_BUFFER_ARB );
			}

			for( j = 0; j < tess.numVertexes; j++ ) {
				glVertex_t *ptr = &tess.vertexPtr[j];
				unsigned short tmpX = (unsigned short)(ptr->xyz[0] / MD3_XYZ_SCALE + 32768.0f);
				unsigned short tmpY = (unsigned short)(ptr->xyz[1] / MD3_XYZ_SCALE + 32768.0f);
				unsigned short tmpZ = (unsigned short)(ptr->xyz[2] / MD3_XYZ_SCALE + 32768.0f);
				unsigned char nrmX = (unsigned char)(ptr->normal[0] * 127 + 128);
				unsigned char nrmY = (unsigned char)(ptr->normal[1] * 127 + 128);
				unsigned char nrmZ = (unsigned char)(ptr->normal[2] * 127 + 128);
		...
		if( qglTexBufferEXT ) {
			...
			qglBindTexture( GL_TEXTURE_BUFFER_EXT, image->texnum );
			qglTexBufferEXT( GL_TEXTURE_BUFFER_EXT,
					 GL_RGBA32F_ARB, image->buffer );
			qglBindTexture( GL_TEXTURE_BUFFER_EXT, 0 );
		} else {
