...
cvar_t  *r_ext_texture_buffer_object;
cvar_t	*r_ext_uniform_buffer_object;
cvar_t	*r_ext_texture3D;
cvar_t	*r_ext_separate_stencil;
cvar_t	*r_ext_debug_output;
cvar_t	*r_ext_gpu_shader4;
cvar_t	*r_ext_blend_func_extended;
...
void APIENTRY R_DebugProc(GLenum source, GLenum type,
...
static void InitOpenGL( void )
{
	...
	if( qglDebugMessageControlARB ) {
		...
		qglDebugMessageInsertARB( GL_DEBUG_SOURCE_APPLICATION_ARB,
					  ...
					  GL_DEBUG_SEVERITY_LOW_ARB,
					  -1, "debug log started" );
		if( r_ext_debug_output->integer & 0x8 ) {
			qglDebugMessageCallbackARB(&R_DebugProc, NULL);
...
void R_Register( void ) 
{
	...
	r_ext_map_buffer_range = ri.Cvar_Get( "r_ext_map_buffer_range", "1", CVAR_ARCHIVE | CVAR_LATCH);
	...
	r_ext_texture_buffer_object = ri.Cvar_Get( "r_ext_texture_buffer_object", "1", CVAR_ARCHIVE | CVAR_LATCH);
	r_ext_uniform_buffer_object = ri.Cvar_Get( "r_ext_uniform_buffer_object", "1", CVAR_ARCHIVE | CVAR_LATCH);
	...
	r_ext_gpu_shader4 = ri.Cvar_Get( "r_ext_gpu_shader4", "1", CVAR_LATCH );
	r_ext_blend_func_extended = ri.Cvar_Get( "r_ext_blend_func_extended", "1", CVAR_LATCH );
	...
	r_minIBOSize = ri.Cvar_Get("r_minIBOSize","1024", 0);
...
void R_Init( void ) {	
	...
	if( sizeof( glVertex_t ) != 48 )
		ri.Printf( PRINT_ERROR, "Incorrect glVertex_t size\n" );
...
//	Swap_Init();
	...
	if( qglGenBuffersARB ) {
		qglGenBuffersARB( 1, &tess.VtxBuf.id );
		qglGenBuffersARB( 1, &tess.IdxBuf.id );
	}
...
void RE_Shutdown( qboolean destroyWindow ) {	
	...
	if ( tr.registered ) {
		...
		GL_VBO( 0 );
		GL_UBO( 0 );
		if ( qglIsBufferARB && tr.clusters ) {
			int cluster, dir;
			for( cluster = 0; cluster < tr.world->numClusters;
			     cluster++ ) {
				for( dir = 0; dir < 7; dir++ ) {
					if( tr.clusters[cluster].numIBOSurfaces[dir] > 0 ) {
						qglDeleteBuffersARB( 1, &tr.clusters[cluster].iboSurfaces[dir][0].ibo.ibo );
					}
				}
			}
		}
		if ( qglIsBufferARB && backEnd.worldVBO ) {
			if ( qglIsBufferARB( backEnd.worldVBO->vbo ) ) {
				...
				ri.Free(backEnd.worldVBO);
				backEnd.worldVBO = NULL;
			}
		}
		if ( qglIsBufferARB && tr.numModels ) {
			...
			for ( mod = 0; mod < tr.numModels; mod++ ) {
				srfMD3Texture_t	*(*data)[MD3_MAX_LODS];
				model = tr.models[mod];
				...
				for ( lod = 0; lod < MD3_MAX_LODS; lod++ ) {
					srfMD3Texture_t *surf;
					if( data && (*data)[lod] != NULL ) {
						int srf;
						for( srf = 0; srf < model->md3[lod]->numSurfaces; srf++ ) {
							surf = &(*data)[lod][srf];
							if( qglIsBufferARB( surf->IBO->ibo ) )
								qglDeleteBuffersARB( 1, &surf->IBO->ibo );
							if( surf->VBO &&
							    qglIsBufferARB( surf->VBO->vbo ) )
								qglDeleteBuffersARB( 1, &surf->VBO->vbo );
						}
					}
				}
			}
		}
		if( qglIsBufferARB && backEnd.dlightBuffer ) {
			if( qglIsBufferARB( backEnd.dlightBuffer ) )
				qglDeleteBuffersARB( 1, &backEnd.dlightBuffer );
			backEnd.dlightBuffer = 0;
		}
		if( qglIsBufferARB && backEnd.fogBuffer ) {
			if( qglIsBufferARB( backEnd.fogBuffer ) )
				qglDeleteBuffersARB( 1, &backEnd.fogBuffer );
			backEnd.fogBuffer = 0;
		}
		if( qglIsBufferARB && tess.VtxBuf.id ) {
			if( qglIsBufferARB( tess.VtxBuf.id ) )
			qglDeleteBuffersARB( 1, &tess.VtxBuf.id );
			tess.VtxBuf.id = 0;
		}			
		if( qglIsBufferARB && tess.IdxBuf.id ) {
			if( qglIsBufferARB( tess.IdxBuf.id ) )
				qglDeleteBuffersARB( 1, &tess.IdxBuf.id );
			tess.IdxBuf.id = 0;
		}			
		...
		Com_Memset( &glState, 0, sizeof(glstate_t) );
	}
...
void RE_EndRegistration( void ) {
	R_SyncRenderThread();
	if (!ri.Sys_LowPhysicalMemory() && GLimp_InBackend() ) {
		RB_ShowImages();
	}
}
