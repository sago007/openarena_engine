...
FT_Bitmap *R_RenderGlyph(FT_GlyphSlot glyph, glyphInfo_t* glyphOut) {
	...
	if ( glyph->format == ft_glyph_format_outline ) {
		...
		bit2 = ri.Hunk_AllocateTempMemory( sizeof(FT_Bitmap) );
		...
		bit2->buffer     = ri.Hunk_AllocTempMemory( pitch * height );
		bit2->num_grays = 256;
...
void WriteTGA (char *filename, byte *data, int width, int height) {
	...
	buffer = ri.Hunk_AllocateTempMemory( width*height*4 + 18 );
...
*/
		...
		if (*yOut + *maxHeight + 1 >= 255) {
			*yOut = -1;
			*xOut = -1;
			ri.Hunk_FreeTempMemory( bitmap->buffer );
			ri.Hunk_FreeTempMemory( bitmap );
			return &glyph;
		}
