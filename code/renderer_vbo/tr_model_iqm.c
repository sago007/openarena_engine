...
static void JointToMatrixInverse( vec4_t rot, vec3_t scale, vec3_t trans,
	...
	inv_scale[0] = 1.0f / scale[0];
	inv_scale[1] = 1.0f / scale[1];
	inv_scale[2] = 1.0f / scale[2];

	mat[ 0] = inv_scale[0] * (1.0f - (yy + zz));
	mat[ 1] = inv_scale[1] * (xy + wz);
	mat[ 2] = inv_scale[2] * (xz - wy);
	mat[ 3] = -DotProduct((mat + 0), trans);
	mat[ 4] = inv_scale[0] * (xy - wz);
	mat[ 5] = inv_scale[1] * (1.0f - (xx + zz));
	mat[ 6] = inv_scale[2] * (yz + wx);
	mat[ 7] = -DotProduct((mat + 4), trans);
	mat[ 8] = inv_scale[0] * (xz + wy);
	mat[ 9] = inv_scale[1] * (yz - wx);
	mat[10] = inv_scale[2] * (1.0f - (xx + yy));
...
qboolean R_LoadIQM( model_t *mod, void *buffer, int filesize, const char *mod_name ) {
	...
	for( i = 0; i < header->num_joints; i++, joint++ ) {
		...
		JointToMatrixInverse( joint->rotate, joint->scale, joint->translate, invBaseFrame );
...
void RB_IQMSurfaceAnim( tessMode_t mode, surfaceType_t *surface ) {
	...
	if( mode & TESS_VERTEX ) {
		glVertex_t	*ptr = tess.vertexPtr + tess.numVertexes;
		...
		for( i = 0; i < surf->num_vertexes; i++, ptr++ ) {
			int	j, k;
			float	vtxMat[12];
			float	nrmMat[9];
			...
			ptr->tc1[0] = data->texcoords[2*vtx + 0];
			ptr->tc1[1] = data->texcoords[2*vtx + 1];
			ptr->tc2[0] = ptr->tc1[0];
			ptr->tc2[1] = ptr->tc1[1];
			...
			ptr->color[0] = data->colors[4*vtx+0];
			ptr->color[1] = data->colors[4*vtx+1];
			ptr->color[2] = data->colors[4*vtx+2];
			ptr->color[3] = data->colors[4*vtx+3];
		}
	...
	if( mode & TESS_INDEX ) {
		glIndex_t	*ptr = tess.indexPtr + tess.numIndexes[tess.indexRange];
	...
	tess.numIndexes[tess.indexRange] += 3 * surf->num_triangles;
	if( tess.minIndex[tess.indexRange] > tess.numVertexes )
		tess.minIndex[tess.indexRange] = tess.numVertexes;
	tess.numVertexes += surf->num_vertexes;
	if( tess.maxIndex[tess.indexRange] > tess.numVertexes - 1 )
		tess.maxIndex[tess.indexRange] = tess.numVertexes - 1;
}
